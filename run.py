...
class clear_data:
    def __init__(self):
        ...
        self.connect = pymysql.Connect(host=self.host, port=int(self.port), user=self.username, passwd=self.passwd, db=self.dbname,
                               charset='utf8')
        self.cur = self.connect.cursor()
    def clear_data(self, dbname):
        sql = "select CONCAT('truncate table ',  table_name, ';')  from information_schema.`TABLES` where TABLE_SCHEMA= %s"
        self.cur.execute(sql,(dbname))
